...
;------------------------------------------------------------------
%define MAX_WINDOWS 5            ; Maximum number of windows
...
; Window structure (22 bytes per window)
struc WINDOW
    ...
    .flags      resb 1    ; Flags (bit 0 = visible, bit 1 = active)
endstruc

; Window tracking data
window_count db 0                                    ; Number of active windows
window_active db 0xFF                                ; ID of the active window (0xFF = none)
window_table: times MAX_WINDOWS * WINDOW_size db 0   ; Window structures

; Window buffer (for saving screen contents)
; Allocated dynamically when creating windows
...
.row_loop:
    ...
    ; Move to next row
    sub di, [si + WINDOW.width]   ; Back to start of window
    sub di, [si + WINDOW.width]
